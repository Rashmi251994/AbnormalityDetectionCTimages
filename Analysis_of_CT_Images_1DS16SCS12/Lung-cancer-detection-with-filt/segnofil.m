...
hy = fspecial('sobel');
hx = hy';
Iy = imfilter(double(dicomread_image2 ), hy, 'replicate');
Ix = imfilter(double(dicomread_image2 ), hx, 'replicate');
...
se = strel('disk', 20);
Io = imopen(dicomread_image2 , se);
figure
imshow(Io), title('Opening (Io)')
Ie = imerode(dicomread_image2 , se);
Iobr = imreconstruct(Ie,dicomread_image2 );
figure
